...
public class MainActivity extends AppCompatActivity implements AbsListView.OnScrollListener {
    ...
    private ImageAdapter mImageApapter;
    ...
    private void initData() {
        ...
        mUrlList = Arrays.asList(imageUrls);

    }

    private void initView() {
        gridView = findViewById(R.id.gridview);
        mImageApapter = new ImageAdapter(this);
        gridView.setAdapter(mImageApapter);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            gridView.setOnScrollListener(this);
        }
        mImageApapter.setmUrlList(mUrlList);
    }

    @Override
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        if (scrollState == AbsListView.OnScrollListener.SCROLL_STATE_IDLE) {
            mImageApapter.setmIsGridViewIdle(true);
            mImageApapter.notifyDataSetChanged();
        } else {
            mImageApapter.setmIsGridViewIdle(false);
        }
    }
